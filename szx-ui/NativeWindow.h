...
{
    public:
        NativeWindow(): m_hwnd(NULL), m_oldWindowProc(::DefWindowProc), m_isRootWindow(FALSE) {};
    ...
    public:
        ...
        void GetWindowText(std::wstring& wstWindowText);

        void PostMessage(UINT message, WPARAM wParam  = 0, LPARAM lParam  = 0);

        void AsRootWindow() { m_isRootWindow = TRUE;};
