...
void NativeWindow::GetWindowText(std::wstring& wstWindowText)
{
    int length = ::GetWindowTextLength(m_hwnd) + 1;
    wchar_t *tempText = static_cast<wchar_t *>(_alloca(length * sizeof(wchar_t)));
    ::GetWindowText(m_hwnd, tempText, length);
    wstWindowText = tempText;
}

void NativeWindow::PostMessage(UINT message, WPARAM wParam /* = 0  */, LPARAM lParam /* = 0 */)
{
    assert(::IsWindow(m_hwnd));
